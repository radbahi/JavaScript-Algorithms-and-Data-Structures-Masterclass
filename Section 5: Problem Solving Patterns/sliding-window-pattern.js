...
function maxSubarraySum(arr, num) { // naive solution. time complexity = O(n^2)
    if (num > arr.length) { // obviously if the number is greater than the length then we can't calculate this
        return null
    }
    let max = -Infinity // set to -infinity in case the largest sum would still be a negative
    for (let i = 0; i < arr.length - num + 1; i++) { // i < arr.length - num + 1 to make sure the subarray's length doesn't go past the actual array's length
        let temp = 0 // stores our sum each loop
        for (let j = 0; j < num; j++) { // loops up to the num
            temp += arr[i + j] // adds to the sum from each element in subarray
        }
        if (temp > max) { // if higher than previous max value...
...
function maxSubarraySum(arr, num) { // refactored. time complexity = O(n)
    let maxSum = 0
    let tempSum = 0
    if (num > arr.length) { // obviously if the number is greater than the length then we can't calculate this
        return null
    }
    for (let i = 0; i < num; i++) { // get an initial max value by just doing one subarray
        maxSum += arr[i] // calculate it here
    }
    tempSum = maxSum // make these the same value for now
    for (let i = num; i < arr.length; i++) { // let i = num to get the index at the end of the subarray and then continue onto end of whole array
